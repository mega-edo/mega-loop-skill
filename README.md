...
**`/mega-loop:trace-analyze`** — *"are my traces good enough?"* It grades them against the readiness contract MEGA Loop runs internally and hands back the exact fix for every finding, ordered by how many traces each one clears. Two of its fourteen checks go beyond that contract and can never fail a trace: they ask whether a trace MEGA Loop *can* read actually holds anything worth detecting.
